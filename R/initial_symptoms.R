...
sx_case_nofever<-function(VHFlongdata=VHFcase_recode){
  df<-VHFlongdata[ VHFcase_recode$CaseStatus=="Confirmed",]
  
}
  
nofever$Feverr<-ifelse(is.na(nofever$Fever), "4", nofever$Fever)

ggplot(nofever, aes(x=dDateReport,  fill=factor(Feverr)))+geom_histogram(binwidth=7)+
  scale_fill_brewer(type="qual",palette=2) +
 theme_minimal()


ggplot(VHFcase_recode[ !(VHFcase_recode$CaseStatus == "Not a case"),], 
       aes(x=dReport,  fill=CaseStatus))+geom_histogram(binwidth=7)+
  scale_fill_brewer(type="qual",palette=2) +
  theme_minimal()

ggplot(VHFcase_recode[ !(VHFcase_recode$CaseStatus == "Not a case"),], 
       aes(x=dDateReport,  fill=SCRes))+geom_histogram(binwidth=7)+
  scale_fill_brewer(type="qual",palette=2) +
  theme_minimal()


ggplot(nofever, aes(x=dDateReport,  fill=StatusReport))+
  geom_histogram(binwidth=7)+
  scale_fill_brewer(type="qual",palette=2)+
  theme_minimal()+facet_wrap(~DistrictOnset)
...
VHFalive <- VHFcase_recode[ VHFcase_recode$CaseStatus %in% c("Probable","Confirmed") & VHFcase_recode$StatusReport==2 & !is.na(VHFcase_recode$CaseStatus) & !is.na(VHFcase_recode$StatusReport),]
VHFalive$dateplausible<-ifelse(VHFalive$onsettoreport >=0 & VHFalive$onsettoreport <=14, 1,0)
table(VHFalive$dateplausible, useNA="always")


VHFalive <- VHFcase_recode[ VHFcase_recode$CaseStatus %in% c("Not a case","Confirmed") & VHFcase_recode$StatusReport==2 & !is.na(VHFcase_recode$CaseStatus) & !is.na(VHFcase_recode$StatusReport),]

with(VHFcase_recode[ VHFcase_recode$StatusReport==2,], table(StatusReport, onsettoreport)) 

with(VHFcase_recode[ VHFcase_recode$StatusReport==2,], tabl
     
VHFcase_recode$c_f<-with(VHFcase_recode, ifelse(contact == "Contact reported" & Fever_r==1, 1, 0))
with(VHFcase_recode[ VHFcase_recode$StatusReport == 2, ], table(c_f, CaseStatus, useNA="always"))


#############
VHFcase_recode$Case_Status<-factor(VHFcase_recode$CaseStatus, levels = c("Confirmed","Probable","Suspect","Not a case"))

g1<-ggplot(data= VHFcase_recode, aes(x=dDateReport, fill=Case_Status))+geom_histogram(binwidth=30)+
  scale_fill_manual(values=c("#e31a1c","#fb9a99","#b2df8a","#1f78b4")) + theme_bw()
  

g2<-ggplot(data= VHFcase_recode, aes(x=dDateReport, fill=Case_Status))+
  geom_histogram(binwidth=30)+
  scale_fill_manual(values=c("#e31a1c","#fb9a99","#b2df8a","#1f78b4")) +
facet_wrap(~SCRes)+theme_bw()


pdf("c:/vhfdata/results/epicurve.pdf")
  print(g1)
  print(g2)
  dev.off()
